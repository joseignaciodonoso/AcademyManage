...
export default async function ClubLayout({ children }: { children: React.ReactNode }) {
  ...
  // Fetch organization type from academy
  let organizationType: "ACADEMY" | "CLUB" = "ACADEMY"
  try {
    const academyId = (session.user as any).academyId as string | undefined
    if (academyId) {
      const academy = await prisma.academy.findUnique({ where: { id: academyId }, select: { type: true } })
      if (academy?.type) organizationType = academy.type as any
    }
  } catch {}
          ...
          <header className="flex h-16 items-center gap-4 border-b border-border bg-gradient-to-r from-[hsl(var(--background))] via-[hsl(var(--muted))] to-[hsl(var(--background))] backdrop-blur-md px-4 lg:px-6 shadow-lg">
            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="flex flex-col">
                <h2 className="text-base sm:text-lg font-semibold text-[hsl(var(--foreground))] leading-tight">Panel del Club</h2>
                <p className="text-xs text-slate-300 font-medium">Administración</p>
              </div>
            </div>
